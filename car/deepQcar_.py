...
allactions=[np.array(list(k)) for k in it.product([-1, 0, 1], [1, 0], [0.2, 0])]

class DQNAgent():
    def __init__(self,actions=4,obs=8):
        self.actions=len(allactions)
        self.observations=(96, 96, 3)
    ...
    def play(self,observation,epsilon):
        if (len(self.memory)<BATCHSIZE):
            ind=np.random.randint(low=0,high=self.actions)
            action=allactions[ind]
            return action
        else:
...
#         print(data.shape,len(errors))
    ...
    def train_model(self):
        ...
        qtable_to_update=self.target_model.predict(state)
        def findIndex(a):
            return [(i,t) for i,t in enumerate(allactions) if np.all(t==a)][0][0]
        for indx,qs in enumerate(qtable_to_update):
            qtable_to_update[indx,findIndex(action[indx])]=qnext_max[indx]
#         print(data[:5])
    ...
    def model_predictions(self,observation):
        pred=self.model.predict(observation.reshape(1,96,96,3))
        pred=np.argmax(pred)
        return allactions[pred]
        
    def load_model(self):
        num_input = layers.Input(shape=(self.observations))
        x=layers.Conv2D(32,kernel_size=3, activation="relu")(num_input)
...
def gen_video(ag,epoch):
    ...
    env = gym.wrappers.Monitor(env,"vids/{}".format(epoch))
...
for i_episode in range(1, n_episodes+1):
    ...
    if i_episode % 10 == 0:
        agent.model.save_weights("./weightsfolder/car_weights_{}.h5".format(i_episode))
        gen_video(agent,i_episode)
    if score>best_score_so_far:
        best_score_so_far=score
        agent.model.save_weights("./weightsfolder/car_best_weights_.h5")
...
plt.ylabel('Epsilone')
plt.xlabel('Episode #')
plt.savefig("epsiloncarFT.png")
# plt.show()  
agent.model.save_weights("./weightsfolder/car_agent_weights.h5")